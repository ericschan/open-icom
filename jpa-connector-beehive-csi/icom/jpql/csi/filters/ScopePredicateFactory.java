...
import oracle.csi.filters.QuotaSortCriteria;
...
public abstract class ScopePredicateFactory extends EntityPredicateFactory {
	...
	{
		sortCriteriaMethodMapper.put("HardQuota".toLowerCase(), "createQuotaSortCriteria");
		sortCriteriaMethodMapper.put("AllocatedQuota".toLowerCase(), "createQuotaSortCriteria");
	}
	...
	public SortCriteria createSortCriteria(Object listFilter, OrderByItem item) {
		if (item.simpleAttributeName.equalsIgnoreCase("HardQuota")) {
			...
			args[0] = QuotaSortCriteria.QuotaType.valueOf("HARD_QUOTA");
			args[1] = new Boolean(item.ascending);
			...
			Method method = getSortCriteriaMethod(listFilter, item);
			Object[] args = new Object[2];
			args[0] = QuotaSortCriteria.QuotaType.valueOf("ALLOCATED_QUOTA");
			args[1] = new Boolean(item.ascending);
			SortCriteria sortCriteria = ListFilterHelper.getInstance().createSortCriteria(method, listFilter, args);
			return sortCriteria;
		} else {
			return super.createSortCriteria(listFilter, item);
		}
	}
