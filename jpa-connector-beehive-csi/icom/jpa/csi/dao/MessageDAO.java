...
public abstract class MessageDAO extends ArtifactDAO {
	...
	public void copyObjectState(ManagedObjectProxy obj, Object csiEntity, Projection proj) {
		...
		Message csiMessage = (Message) csiEntity;
		...
		if (!isPartOfProjection(MessageInfo.Attributes.deliveredTime.name(), lastLoadedProjection) &&
				isPartOfProjection(MessageInfo.Attributes.deliveredTime.name(), proj)) {
			try {
				DateTime dt = csiMessage.getDeliveredTime();
				if (dt != null) {
					assignAttributeValue(pojoIdentifiable, MessageInfo.Attributes.deliveredTime.name(), dt.getTimestamp());
				} else {
	...
	private void updateNewOrOldObjectState(ManagedIdentifiableProxy obj, DAOContext context) {
		...
		oracle.bom.ejb.spi.Identifiable pojoMessage = obj.getPojoIdentifiable();
		*/
	}
