...
public class EmailParticipantUtil {
	...
	static public void copyObjectState(ManagedObjectProxy obj, Object csiObject, Projection proj) {
		EmailParticipant csiParticipant = (EmailParticipant) csiObject;
	...
	static EmailParticipant updateEmailParticipant(Object pojoParticipant) {
		EmailParticipant csiParticipant = null;
		...
		if (pojoAddressable != null) {
			...
			if (fullAddress != null) {
				...
				csiParticipant = new EmailParticipant(addressable, inetMailAddress);
			} else if (address != null) {
				Addressable addressable = (Addressable) EntityUtils.getInstance().createEmptyEntity(id);
				InetMailAddress inetMailAddress = new InetMailAddress(address.toString());  // TODO
				csiParticipant = new EmailParticipant(addressable, inetMailAddress);
			} else {
				AddressableHandle addressableHandle = (AddressableHandle) EntityUtils.getInstance().createHandle(id);
				csiParticipant = new EmailParticipant(addressableHandle);
			}
			return csiParticipant;
		} else if (fullAddress != null) {
			InetMailAddress inetMailAddress = new InetMailAddress(fullAddress);
			csiParticipant = new EmailParticipant(inetMailAddress);
		} else if (address != null) {
			InetMailAddress inetMailAddress = new InetMailAddress(address.toString()); // TODO
			csiParticipant = new EmailParticipant(inetMailAddress);
		} else {
