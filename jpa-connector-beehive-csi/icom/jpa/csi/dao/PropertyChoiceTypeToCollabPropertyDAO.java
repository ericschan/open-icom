...
public class PropertyChoiceTypeToCollabPropertyDAO extends CsiAbstractDAO {
	...
	public void copyObjectState(ManagedObjectProxy managedObj, Object csiProperty, Projection proj) {
		copyObjectState(managedObj.getPojoObject(), (CollabProperty) csiProperty);
	}
	...
	public void updateObjectState(Object pojoObject, CollabPropertiesUpdater updater, Operand operand) {
		...
		if (operand == Operand.ADD) {
			updater.addProperty(name, description, value);
		} else if (operand == Operand.REMOVE) {
			updater.removeProperty(name);
