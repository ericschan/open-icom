...
public abstract class CommunityDAO extends ScopeDAO {
	...
	public void copyObjectState(ManagedObjectProxy obj, Object bdkEntity, Projection proj) {
		...
		//Community bdkCommunity = (Community) bdkEntity;
	}
    ...
    public void loadAndCopyObjectState(ManagedIdentifiableProxy obj, String attributeName, Object key) {
        ...
        if (CommunityInfo.Attributes.actors.name().equals(attributeName)) {
            ...
            actors = OrganizationUserDAO.getInstance().loadUsers(obj, communityId, Projection.EMPTY);
            mergeAssignIdentifiables(obj, CommunityInfo.Attributes.actors.name(), actors);
        }

        if (CommunityInfo.Attributes.communities.name().equals(attributeName)) {
            Set<Persistent> subCommunities = null;
            subCommunities = OrganizationDAO.getInstance().loadCommunities(obj, Projection.EMPTY);
            mergeAssignIdentifiables(obj, CommunityInfo.Attributes.communities.name(), subCommunities);
        }

        if (CommunityInfo.Attributes.spaces.name().equals(attributeName)) {
            Set<Persistent> spaces = null;
            spaces = WorkspaceDAO.loadSpaces(obj, communityId, Projection.EMPTY);
            mergeAssignIdentifiables(obj, CommunityInfo.Attributes.spaces.name(), spaces);
        }
    }
