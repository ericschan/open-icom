...
public class BdkQueryContextImpl implements BdkQueryContext {
    ...
    public List<Object> getResultList(Object cacheContext) {
    	...
    	if (maxResult >= 0) {
    		predicateAndSortListParameters.setCountLimit(maxResult);
		}
    	predicateAndSortListParameters.setPredicate(finalPredicate);
    	predicateAndSortListParameters.setSortCriteria(sortCriteria);
    	
    	String resourceType = predicateFactory.getResourceType();
    	BeeId id = null;  // TODO
    	String resourceId = predicateFactory.getResourceId(id);
		...
		String query = "";
		if (predicateFactory.hasContainerAsArgumentOfListMethod()) {
			query += "parent=" + container.getId();
		} else if (predicateFactory.hasWorkspaceAsArgumentOfListMethod()) {
			query += "workspace=" + workspace.getId();
		} else if (predicateFactory.hasMarkerAsArgumentOfListMethod()) {
			query += "marker=" + marker.getId();
		}
		
		List<Object> list = BdkAbstractDAO.listEntities((PersistenceContextImpl) cacheContext, Entity.class, 
									predicateAndSortListParameters, resourceType, resourceId, projection, query);
		return list;
	}
    ...
    public Node visitNode(WhereClauseNode node, AbstractNode parent) {
		...
		if (lastExpression instanceof ConditionalTerm) {
			...
			term.predicate = predicateFactory.createMatchAnyPredicate(matchAnyPredicateList);
			...
			if (matchAllPredicateList.size() == 1) {
				...
				factor.predicate = predicateFactory.createMatchAllPredicate(matchAllPredicateList);
			}
		}
	...
	public Node visitNode(NotNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createNotPredicate((Predicate) expression.predicate);
	...
	public Node visitNode(EqualsNode node, AbstractNode parent) {
		...
		if (currentExpression.primary.simpleAttributeName.equalsIgnoreCase("parent") &&
			...
			if (currentExpression.firstOperand.value instanceof BeeId) {
				container = (BeeId) currentExpression.firstOperand.value;
			} else if (currentExpression.firstOperand.value instanceof String) {
				String str = (String) currentExpression.firstOperand.value;
		    	container = BdkDataAccessUtilsImpl.getBeeId(str);
			}
			...
			if (currentExpression.firstOperand.value instanceof BeeId) {
				marker = (BeeId) currentExpression.firstOperand.value;
			} else if (currentExpression.firstOperand.value instanceof String) {
				String str = (String) currentExpression.firstOperand.value;
				marker = BdkDataAccessUtilsImpl.getBeeId(str);
			}
			currentExpression.shouldNotHaveInterveningOrNodes = true;
			...
			if (currentExpression.firstOperand.value instanceof BeeId) {
				workspace = (BeeId) currentExpression.firstOperand.value;
			} else if (currentExpression.firstOperand.value instanceof String) {
				String str = (String) currentExpression.firstOperand.value;
				workspace = BdkDataAccessUtilsImpl.getBeeId(str);
			}
			currentExpression.shouldNotHaveInterveningOrNodes = true;
			shouldNotHaveMoreThanOneWorkspacePredicate = true;
		} else {
			predicate = predicateFactory.createEqualsPredicate(currentExpression);
		}
	...
	public Node visitNode(NotEqualsNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createNotEqualsPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(LikeNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createLikePredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(BetweenNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createBetweenPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(GreaterThanEqualToNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createGreaterThanOrEqualsPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(LessThanEqualToNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createLessThanOrEqualsPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(GreaterThanNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createGreaterThanPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(LessThanNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createLessThanPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(MemberOfNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createMemberOfPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
	...
	public Node visitNode(InNode node, AbstractNode parent) {
		...
		Predicate predicate = predicateFactory.createInPredicate(currentExpression);
		currentExpression.predicate = predicate;
		expressionStack.push(currentExpression);
		currentExpression = null;
		return node;
    ...
    public Node visitNode(OrderByClauseNode node, AbstractNode parent) {
		...
		if (orderByItems.size() == 1) {
			...
			sortCriteria = predicateFactory.createSortCriteria(orderByItems);
		} else {
			throw new QLException("Not Supported");
		}
		return node;
    ...
    public Node visitNode(OrderByItemNode node, AbstractNode parent) {
    	...
    	SortCriteria sortCriteria = predicateFactory.createSortCriteria(item);
