...
public class JpaModifyMethodAdapter extends JpaMethodAdapter implements Descriptors {
    ...
    public JpaModifyMethodAdapter(MethodVisitor mv, ClassDefinition classDefn, FieldDefinition fieldDefn,
    ...
    public void visitCode() {
        if (wellFormedAccessor) {
            if (! fieldDefn.deferLoadOnAddRemove) {
             ...
             // invoke preModifyEmbedded("fieldX", [argX])
                mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                        preModifyEmbeddedMethodName, preModifyEmbeddedMethodDesc);
             // stack has ()
    ...
    public void visitInsn(int code) {
        ...
        if (wellFormedAccessor) {
            /* inject the following code before return(r)
               if (r)
                  postModifyEmbedded("fieldX", [argX], [argX])
             */
         // stack has (..., r)
         // DUP r
            mv.visitInsn(DUP);
        ...
        // invoke postModifyEmbedded("fieldX", [argX], [argX])
            mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                   postModifyEmbeddedMethodName, postModifyEmbeddedMethodDesc);
         // stack has (..., r)
         // label:
            mv.visitLabel(label);
         // stack has (..., r)
