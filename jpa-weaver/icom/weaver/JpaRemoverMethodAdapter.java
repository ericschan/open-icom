...
public class JpaRemoverMethodAdapter extends JpaMethodAdapter {
    ...
    public JpaRemoverMethodAdapter(MethodVisitor mv, ClassDefinition classDefn, FieldDefinition fieldDefn,
            ...
            if (! fieldDefn.deferLoadOnAddRemove) {
             ...
             // stack has ([this], "fieldX", [argX])
                if (fieldDefn.isEntity()) {
                 // invoke preRemoveReference("fieldX", [argX])
                    mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                            preRemoveReferenceMethodName, preRemoveReferenceMethodDesc);
                } else if (fieldDefn.isEmbedded()) {
                 // invoke preRemoveEmbedded("fieldX", [argX])
                    mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                            preRemoveEmbeddedMethodName, preRemoveEmbeddedMethodDesc);
                } else {
                 // invoke preRemoveReference("fieldX", [argX])
                    mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                            preRemoveReferenceMethodName, preRemoveReferenceMethodDesc);
    ...
    public void visitInsn(int code) {
        ...
        if (wellFormedAccessor) {
            /* inject the following code before return(r)
               if (r)
                  postRemoveReference("fieldX", [argX], [argX])
             */
         // stack has (..., r)
         // DUP r
            mv.visitInsn(DUP);
         ...
         // stack has (..., r, [this], "fieldX", [argX], [argX])
            if (fieldDefn.isEntity()) {
            // invoke postRemoveReference("fieldX", [argX], [argX])
                mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                       postRemoveReferenceMethodName, postRemoveReferenceMethodDesc);
            } else if (fieldDefn.isEmbedded()) {
            // invoke postRemoveEmbedded("fieldX", [argX], [argX])
                mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                       postRemoveEmbeddedMethodName, postRemoveEmbeddedMethodDesc);
            } else {
            // invoke postRemoveReference("fieldX", [argX], [argX])
                mv.visitMethodInsn(INVOKEVIRTUAL, classDefn.internalClassName, 
                       postRemoveReferenceMethodName, postRemoveReferenceMethodDesc);
