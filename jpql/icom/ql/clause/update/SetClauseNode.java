...
public class SetClauseNode extends ClauseNode {
	
    private List assignmentNodes = null;

    public SetClauseNode() {
        super();
    ...
    public Node accept(ParseTreeVisitor visitor, AbstractNode parent) {
    	for (int i = 0; i < assignmentNodes.size(); i++) {
    		Node item = (Node)assignmentNodes.get(i);
    		Node newItem = item.accept(visitor, this);
    		if (newItem != item) {
    			assignmentNodes.set(i, newItem);
            }
    	}
