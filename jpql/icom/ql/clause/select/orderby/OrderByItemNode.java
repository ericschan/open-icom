 ...
 * holder.
 */
package icom.ql.clause.select.orderby;
...
public class OrderByItemNode extends Node {
    ...
    public Node accept(ParseTreeVisitor visitor, AbstractNode parent) {
    	if (orderByItem != null) {
    		Node newOrderByItem = orderByItem.accept(visitor, this);
    		if (newOrderByItem != orderByItem) {
    			orderByItem = newOrderByItem;
    		}
    	}
    	if (direction != null) {
    		direction = (SortDirectionNode) direction.accept(visitor, this);
    	}
        return visitor.visitNode(this, parent);
    }
    
    public String resolveAttribute() {
        return orderByItem.resolveAttribute();
    }
