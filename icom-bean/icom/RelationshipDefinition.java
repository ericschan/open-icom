...
@XmlType(name="RelationshipDefinition", namespace="http://docs.oasis-open.org/ns/icom/metadata/201008")
@XmlAccessorType(XmlAccessType.FIELD)
@XmlRootElement(namespace="http://docs.oasis-open.org/ns/icom/metadata/201008")
public class RelationshipDefinition extends Entity implements RelationshipBondable {
	
	@XmlElement(namespace="http://docs.oasis-open.org/ns/icom/core/201008")
	String description;

    @XmlElement(name="allowedSourceType")
	Collection<URI> allowedSourceTypes;

    @XmlElement(name="allowedTargetType")
	Collection<URI> allowedTargetTypes;
	...
	public RelationshipDefinition(Id id, Extent parent, Date createdOn) {
		super(id, parent, createdOn);
	}
	
	public RelationshipDefinition(Extent parent, Date createdOn) {
	...
	public Collection<URI> getAllowedSourceTypes() {
		if (allowedSourceTypes != null) {
			return Collections.unmodifiableCollection(new HashSet<URI>(allowedSourceTypes));
		} else {
			return null;
		}
	}
	
	public void setAllowedSourceTypes(Collection<URI> allowedSourceTypes) {
		this.allowedSourceTypes = new ArrayList<URI>(allowedSourceTypes);
	}
	
	public Collection<URI> getAllowedTargetTypes() {
		if (allowedTargetTypes != null) {
			return Collections.unmodifiableCollection(new HashSet<URI>(allowedTargetTypes));
		} else {
			return null;
		}
	}
	
	public void setAllowedTargetTypes(Collection<URI> allowedTargetTypes) {
		this.allowedTargetTypes = new ArrayList<URI>(allowedTargetTypes);
	}
